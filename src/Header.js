import React from 'react'
import "./Header.css"
import SearchIcon from '@mui/icons-material/Search';
import HeaderOption from './HeaderOption';
import { BusinessCenter, Chat, HomeOutlined, Notifications, SupervisorAccount } from '@mui/icons-material';


function Header() {
  return (
    <div className="header">
        <div className="header__left">
            <img src="data:image/png;base64," alt="" />
            <div className="header__search">
                <SearchIcon />
                <input type="text" />
            </div>
        </div>
        <div className="header__right">
            <HeaderOption title="Home" Icon = {HomeOutlined}/>
            <HeaderOption title="My Network" Icon = {SupervisorAccount}/>
            <HeaderOption title="jobs" Icon = {BusinessCenter}/>
            <HeaderOption title="Messaging" Icon = {Chat}/>
            <HeaderOption title="Notifications" Icon = {Notifications}/>
            <HeaderOption avatar="https://media-exp1.licdn.com/dms/image/C5603AQEU1G479IKZcg/profile-displayphoto-shrink_200_200/0/1644339604236?e=1667433600&v=beta&t=gmqgB5nFtIw1f3T0l3WeKKESaTz8oJCaCyoDMxJe60o" title = "Me"/>
        </div>
    </div>
  )
}

export default Header